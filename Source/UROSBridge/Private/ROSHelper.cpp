...
geometry_msgs::Point	ConvertPointUE4ToROS(const FVector &Vector)
{
	return geometry_msgs::Point(0.01*Vector.X, -0.01*Vector.Y, 0.01*Vector.Z);
}

FVector	FROSHelper::ConvertPointROSToUE4(const geometry_msgs::Point &point)
{
	return FVector(100.0*point.GetX(), -100.0*point.GetY(), 100.0*point.GetZ());
}
